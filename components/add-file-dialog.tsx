        ...
        <DialogContent className="w-[min(96vw,72rem)] max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          {canAddMore ? (
            <Tabs value={browseTab} onValueChange={handleTabChange} className="w-full flex-1 min-h-0">
              ...
              <TabsContent value="upload" className="space-y-4 mt-4 flex-1 min-h-0 overflow-y-auto pr-1">
                <div
                  {...getRootProps()}
                ...
                >
                  ...
                  <p className="text-sm font-medium">
                    {trmlDocumentSetup("dragDropFiles") || "Drag and drop files here"}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {trmlTranslate("or") || "or"} <span className="text-primary font-semibold">{trmlCommon("browseFiles") || "browse"}</span>
                  </p>
                </div>
              ...
              <TabsContent value="browse" className="space-y-4 mt-4 flex-1 min-h-0 overflow-y-auto pr-1">
                {browseError && (
                  <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/30 flex items-start gap-2">
                    <AlertTriangle className="w-4 h-4 text-destructive flex-shrink-0 mt-0.5" />
                ...
                {browseLoading ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between gap-3 pt-1">
                      <Button type="button" variant="outline" onClick={() => handleBrowsePageChange(browsePage - 1)} disabled={browseLoading || browsePage <= 1}>
                        {trmlCommon("previous") || "Previous"}
                      </Button>
                      <div className="text-xs text-muted-foreground">
                        {trmlCommon("pageOf", { current: browsePage, total: browseTotalPages }) || `Page ${browsePage} of ${browseTotalPages}`}
                      </div>
                      <Button type="button" variant="outline" onClick={() => handleBrowsePageChange(browsePage + 1)} disabled={browseLoading || browsePage >= browseTotalPages}>
                        {trmlCommon("next") || "Next"}
                      </Button>
                    </div>
                  </>
                )}
